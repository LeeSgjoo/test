...
public class studentDAO {
    ...
    public int insertStudent(studentVO vo) {
        ...
        return jdbcTemplate.update(STUDENT_INSERT,
                vo.getName(),
                vo.getAge(),
                vo.getMajor(),
                vo.getRc(),
    ...
    /** 2. 학생 목록 조회 (READ LIST) **/
    public List<studentVO> getStudentList() {
        // JdbcTemplate.query() 메서드는 SELECT 쿼리를 실행하고,
        // 결과를 StudentRowMapper를 통해 List<studentVO>로 변환하여 반환합니다.
        return jdbcTemplate.query(STUDENT_LIST, new StudentRowMapper());
    }

    /** (선택) 3. 특정 학생 상세 조회 (READ ONE) **/
    public studentVO getStudent(int id) {
        // queryForObject() 메서드는 단일 행을 반환할 때 사용합니다.
        // ID를 매개변수로 전달합니다.
        return jdbcTemplate.queryForObject(STUDENT_GET, new StudentRowMapper(), id);
    }

    /** (선택) 4. 학생 데이터 수정 (UPDATE) **/
    public int updateStudent(studentVO vo) {
        return jdbcTemplate.update(STUDENT_UPDATE,
                vo.getName(),
                vo.getAge(),
                vo.getMajor(),
                vo.getRc(),
    ...
    /** (선택) 5. 학생 데이터 삭제 (DELETE) **/
    public int deleteStudent(int id) {
        return jdbcTemplate.update(STUDENT_DELETE, id);
    }
}